...
class TextParser(merchantFileWorker: FileWorker) {

    val merchantFileWorker = merchantFileWorker
    ...
    private fun parseMerchant(textBlocks: List<TextBlock>): String? {
        val merchantStringTemplates = merchantFileWorker.getFileList()
    ...
    private fun parseTotalByString(
        textBlocks: List<TextBlock>,
        similarityRate: Float = 0.8f
    ): TextBlock? {
        ...
        for (totalString in totalStringTemplates) {
            ...
            var bestDistance =
                calculateLevenshteinDistance(suitableTextBlocks[0].text.lowercase(), totalString)
    ...
    private fun searchSameRowTotal(
        recognizedText: List<TextBlock>,
        totalTextBlock: TextBlock
    ): Float? {
        ...
        val lowerBlock =
            if (recognizedText[index + 1].boundingBox!!.top < lowerTopLimit) recognizedText[index + 1] else null
    ...
    private fun calculateLevenshteinDistance(recognizedString: String, targetString: String): Int {
        ...
        if (targetStringLength == 0) {
            ...
            return calculateLevenshteinDistance(
                recognizedString.substring(1),
                targetString.substring(1)
            )
        } else {
            return 1 + min(
